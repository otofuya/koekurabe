...
import { C, F, SH, icon, num, mark, phone, browser } from './libc.mjs';

export const BW = 1600, BH = 1000, TOTAL = 16;

export function frame(n, title, body, { sub = '', src = 'cat' } = {}) {
  const SRC = {
    cat: 'カテゴリの数：オーナーから受け取った調査資料（マイベスト・価格.com）から数えた',
    meas: '3カテゴリ：楽天で8商品×30件を単語の規則で数えた見本（AI 分類ではない）。イヤホン：data/ の実データ',
    ear: 'イヤホン：data/ の実データ。ほかのカテゴリは見本（docs/09）',
    none: '未測定の設計案。数字は推定と書いたものだけ',
  }[src];
  return `<div style="position:relative; width:${BW}px; height:${BH}px; overflow:hidden; background:${C.paper}; font-family:${F.jp}; color:${C.ink};">
<div style="position:absolute; left:72px; top:50px; display:flex; align-items:baseline; gap:16px; z-index:10;">${num(`C${n}`, { size: 15, color: C.muted })}<span style="font-weight:700; font-size:32px; line-height:1.2; white-space:nowrap; letter-spacing:.01em;">${title}</span>${sub ? `<span style="font-size:15px; color:${C.ink2}; white-space:nowrap;">${sub}</span>` : ''}</div>
<div style="position:absolute; right:72px; top:58px; z-index:10; display:flex; align-items:center; gap:12px; font-size:12px; color:${C.muted};">${mark(15)}<span>別案C ・ 全ジャンル前提 ・ 2026-09</span></div>
${body}
...
export const kick = (t, color = C.muted) => `<div style="font-family:${F.mono}; font-size:12px; font-weight:500; color:${color}; letter-spacing:.08em;">${t}</div>`;
export const dot = (n, { size = 24, bg = C.ink, fg = '#FFFFFF' } = {}) => `<span style="display:inline-flex; align-items:center; justify-content:center; width:${size}px; height:${size}px; border-radius:50%; background:${bg}; color:${fg}; font-family:${F.mono}; font-weight:500; font-size:${Math.round(size * 0.5)}px; flex-shrink:0;">${n}</span>`;
export const box = (inner, { pad = 18, r = 18, bg = C.card, shadow = SH.card, gap = 10, extra = '' } = {}) => `<div style="box-sizing:border-box; padding:${pad}px; border-radius:${r}px; background:${bg}; box-shadow:${shadow}; display:flex; flex-direction:column; gap:${gap}px; ${extra}">${inner}</div>`;
